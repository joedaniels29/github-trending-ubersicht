options =
  language          : "swift"       # default city in case location detection fails
  days              : "7"              # default region in case location detection fails

appearance =
  iconSet       : 'original'        # "original" for the original icons, or "yahoo" for yahoo icons
  color         : '#fff'            # configure your colors here
  darkerColor   : 'rgba(#CCC, 0.8)'
...
command: "#{process.argv[0]} github-trending.widget/get-trending \
                            \"#{options.language}\" \
                            \"#{options.days}\" "
...
update: (output, domEl) ->
  ...
  return @renderError(data, JSON.stringify(data))
